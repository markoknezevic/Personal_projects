...
class UploadActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if(requestCode == 2 && resultCode == Activity.RESULT_OK && data != null){
            selected = data.data
            try {
                val image = MediaStore.Images.Media.getBitmap(this.contentResolver,selected)
    ...
    fun upload(view: View) {
        ...
        storageReference.putFile(selected!!).addOnSuccessListener(this) {
            ...
            newReference.downloadUrl.addOnSuccessListener { uri ->
                ...
                e -> Toast.makeText(applicationContext, e.localizedMessage, Toast.LENGTH_SHORT).show()
        }
